...
class ENV(gymnasium.Env):
    ...
    def step(self, action):
        toque_tot = scale_action(action[0], torque_bounds)
        k = scale_action(action[1], k_bounds)

        self.prev_energy = self.energy

        toque_f = toque_tot*k
        toque_r = toque_tot*(1-k)

        # self.time = np.array(my_engine.eval('timenow')).reshape(-1) #获取当前仿真时间
        # self.time = self.time[-1]
        self.time += dt

        my_engine.set_param(env_name+'/Torque_f', 'value', str(toque_f), nargout=0)  
        my_engine.set_param(env_name+'/Torque_r', 'value', str(toque_r), nargout=0)
        ...
        reward = -10*abs(dis_spd) - (self.energy-self.prev_energy)*0.1
